...
import { ActivitiesList } from "../../Global/GlobalComponents/Widgets/Activities/ActivitiesList";
...
import { formatDate, getStatusColors } from "../../Global/GlobalResources/Helpers";
...
export const ServPortSubmission = () => {
        ...
        if(submissionData) {
            updateBreadcrumbs({ 
                pageNames: [
                    'Service Portal',
                    'My Items',
                    submissionData.values['Summary']
                ],
                path: `/service-portal/my-items/${kappSlug}/${formSlug}/${submissionData.id}`,
            });
        }
    }, [submissionData]);
    ...
    const submissionsFooter = useMemo(() => {
        return (
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', p: '1.25rem 2.4rem', maxHeight: '2rem', m: '.5rem 0'}}>
                <Button 
                    variant='contained'
    ...
    return submissionData && !pageError ? (
        <>
            <PageTitle title={`${submissionData.label}`} />
            <Box sx={{ display: 'flex', alignItems: 'center', mb: '1rem' }}>
                ...
                <Box sx={{ mr: '1rem' }}><b>Updated at: </b>{formatDate(submissionData.updatedAt, 'MMMM Do, YYYY - h:mm:ss a')}</Box>
